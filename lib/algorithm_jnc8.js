  ...
  generateRecs: function(pt){
    ...
    var filterDrugs = function(curMeds){
      ...
      for(var i = 0; i < curMeds.length; i++){
        //all medication names in meds.medAndClassNames object are lowercase
        var drugName = curMeds[i].medicationName.toLowerCase();
        var className = meds.medAndClassNames[drugName];

        //look up if medication is a relevant drug class
        if(className){
          curMeds[i].className = className;
          filteredDrugs.push(curMeds[i]);
        }
      }
      return filteredDrugs;
    }

    //only interested in drugs of relevant classes
    var relevantDrugs = function(curMeds){
