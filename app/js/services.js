...
angular.module('myApp.services', [])
  ...
  .directive('bpGraph', ['graphHelpers', function(graphHelpers) {
    ...
    return {
      restrict: 'AE',
      template: '<div id="bp-graph" class="graph"></div>',
      link: function (scope, element, attrs) {
        // expects an array for dimensions: [w, h]. will be adjusted for margins.
        // expects an object for targetBP: {systolic: ###, diastolic: ##}
        // expects an array of objects for encounterData
        var dimArr = [600, 400];
        var encounterData = graphHelpers.parseArray(scope.graphData.ptData.db);
        ...
        // let's make a fake timeline!
        var time = [];
        var tempDate = new Date("Tue Apr 08 2014 16:52:13 GMT-0700 (PDT)").getTime();
        for(var i = 0; i < encounterData.length; i++) {
          time.push(tempDate += 9889000);
        }
        ...
        var y = d3.scale.linear().domain([
            graphHelpers.getBPExtreme(encounterData, 'diastolic')-10,
            graphHelpers.getBPExtreme(encounterData, 'systolic')+10
          ]).range([height, 0]);
        ...
        graph.append('svg:path').attr('d', diasLine(data)).attr('class', 'plotline diasLine');
        graph.append('svg:path').attr('d', sysLine(data)).attr('class', 'plotline sysLine');
        graph.append('svg:path').attr('d', diasTargetLine(data)).attr('class', 'plotline diasLine targetLine');
        graph.append('svg:path').attr('d', sysTargetLine(data)).attr('class', 'plotline sysLine targetLine');
      }
    };
  }]);
  ;
