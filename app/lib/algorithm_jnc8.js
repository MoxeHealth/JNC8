...
algorithm.opts = {
  ...
  recMessages: {
    ...
    referralVisit: "Add additional medication class(eg, Beta-blocker, aldosterone antagonist, or others) and/or refer to physician with expertise in hypertension management."
...
algorithm.methods = {
  ...
  runAlgorithm: function(pt){
    ...
    //add the newly generated targetBP to the pt so that it can be saved to the database
    //by the user 
    pt.targetBPs.push(pt.curTargetBP);
  ...
  generateRecs: function(pt){
    ...
    pt.curTargetBP = pt.curTargetBP;
    console.log('curTarget', pt.curTargetBP);

    var filterDrugs = function(curMeds){
      return _.filter(curMeds, function(med){
        //look up if medication is a relevant drug class
        return med.className;
      })
    }

    var drugsatMax = function(curMeds){
      console.log('curMeds');
      return _.every(curMeds, function(med){
        return med.atMax;
      })
    };
    ...
    if(algorithm.methods.isAtBPGoal(pt)) {
      ...
      pt.curMeds = filterDrugs(pt.curMeds);
      ...
      if(!pt.curMeds.length){
        if(!pt.hasCKD) {
          ...
          return {
            ...
            medRecs: meds_jnc8.combos.firstVisit.CKD
          };
        }
      }else{
  ...
  chooseNextMeds: function(curMeds){
    ...
    //strings here must match keys in meds_jnc8.allMeds
    var possibleNextMeds = meds_jnc8.medClasses;
  ...
  isAtBPGoal: function(pt) {
    if(pt.curTargetBP) {
      return pt.curBP.systolic <= pt.curTargetBP.systolic && pt.curBP.diastolic <= pt.curTargetBP.diastolic;
    } else {
